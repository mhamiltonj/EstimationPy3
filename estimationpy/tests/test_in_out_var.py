...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.csvOK = os.path.join(dir_path, "resources", "simpleCSV.csv")
    ...
    def test_measured_output(self):
        ...
        self.assertTrue(self.io_var.is_measured_output(),
                        "The flag measured output has been set to True but value does not correspond")

        self.io_var.set_measured_output(False)
        self.assertFalse(self.io_var.is_measured_output(),
                         "The flag measured output has been set to True but value does not correspond")
    ...
    def test_set_object(self):
        ...
        self.assertIsInstance(self.io_var.get_object(), pyfmi.fmi.ScalarVariable,
                              "The object returned by the GetObject method does not return a pyfmi.fmi.ScalarVariable "
                              "object")
    ...
    def test_set_csv_reader(self):
        ...
        self.assertIsInstance(self.io_var.get_csv_reader(), CsvReader,
                              "The object returned by the GetCsvReader method does not return a CsvReader object")
    ...
    def test_get_data_series_from_csv(self):
        ...
        self.assertIsInstance(self.io_var.get_data_series(), pd.Series,
                              "The reader has a CSV file assigned but not a column, the return value should be a "
                              "pandas.Series")
        self.assertEqual(0, len(self.io_var.get_data_series()),
                         "The reader has a CSV file assigned but not a column, the return value should be a "
                         "pandas.Series empty")
        ...
        self.assertTrue(numpy.allclose(data.values, self.io_var.get_data_series().values),
                        "The pandas Series get is not equal to %s" % str(data.values))
        self.assertListEqual(data.index.tolist(), self.io_var.get_data_series().index.tolist(),
                             "The index of the pandas Series get is not equal to %s" % str(data.index))
    ...
    def test_set_data_series(self):
        ...
        # Compare the values
        self.assertTrue(numpy.allclose(s.values, self.io_var.get_data_series().values),
                        "The pandas Series get is not equal to %s" % str(s.values))
        self.assertListEqual(s.index.tolist(), self.io_var.get_data_series().index.tolist(),
                             "The index of the pandas Series get is not equal to %s" % str(s.index))
    ...
    def test_read_from_data_series(self):
        ...
        for ix in s.index:
            self.assertEqual(s[ix], self.io_var.read_from_data_series(ix),
                             "The value read by the function ReadFromDataSeries does not correspond to the value set")
        ...
        for new_t in new_ts:
            ...
            # Compare the results
            self.assertEqual(int_v, num_int_v,
                             "The interpolated values do not match. Numpy = %.2f , ReadFromDataSeries = %.2f" % (
                                 num_int_v, int_v))
        ...
        for out_t in out_ts:
            ...
            self.assertFalse(self.io_var.read_from_data_series(out_ix),
                             "The index is out of range and the method ReadFromDataSeries has to return False")
