...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.csvNotExisting = os.path.join(dir_path, "resources", "thisFileDoesNotExist.csv")
        self.csvRepeated = os.path.join(dir_path, "resources", "simpleCsvRepeatedValues.csv")
        self.csvUnsorted = os.path.join(dir_path, "resources", "simpleCsvUnsortedValues.csv")
    ...
    def test_instantiate_csv_reader(self):
        ...
        self.assertFalse(self.r.set_selected_column("column_name"),
                         "Not possible to select a column with a missing csv file associated")
    ...
    def test_load_csv_file(self):
        ...
        self.assertListEqual(self.colNames, self.r.get_column_names(),
                             "The column names %s are not the expected ones %s" % (
                                 str(self.r.get_column_names()), str(self.colNames)))
        ...
        self.assertFalse(self.r.open_csv(self.csvNotExisting),
                         "The file %s does not exist and should not be opened" % self.csvNotExisting)

    def test_load_data_series(self):
        # Try to select a data series before assigning the file
        self.assertIsInstance(self.r.get_data_series(), pd.Series,
                              "The reader has not a CSV file assigned, the return value should be a pandas.Series")
        self.assertEqual(0, len(self.r.get_data_series()),
                         "The reader has not a CSV file assigned, the return value should be a pandas.Series empty")
        ...
        self.assertTrue(numpy.allclose(data.values, self.r.get_data_series().values),
                        "The pandas Series get is not equal to %s" % str(data.values))
        self.assertListEqual(data.index.tolist(), self.r.get_data_series().index.tolist(),
    ...
    def test_repeated_data_series(self):
        ...
        self.assertIsInstance(self.r.get_data_series(), pd.Series,
                              "The reader has a file repeated time values, the method should return a pandas.Series")
        self.assertEqual(0, len(self.r.get_data_series()),
                         "The reader has a file repeated time values, the method should return a pandas.Series empty")
